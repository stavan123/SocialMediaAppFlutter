...
return Scaffold(
   ...
   querySnapshot.documents.forEach((doc) {
     notificationItem.add(NotificationsItem.fromDocument(doc));
    });
      ...
      child: Container(
        ...
        child: ListTile(
          title: GestureDetector(
            onTap: () =>
              // displayUserProfile(context,userProfileId:googleSignIn.currentUser.id)
...
configureMediaPreview(context)
{
  if (type == "comment" || type== "like" )
  {
   ...
   onTap:()=> displayOwnProfile(context,userProfileId: googleSignIn.currentUser.id)
,
   child: Container(
     height: 50.0,
     width: 50.0,
...
displayOwnProfile(context,{String userProfileId})
{
 Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => ProfilePage(userProfileId: googleSignIn.currentUser.id)),
  );
}
