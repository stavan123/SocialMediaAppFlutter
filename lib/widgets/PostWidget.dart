...
class _PostState extends State<Post> {
    ...
    builder: (mContext)
    {
      return SimpleDialog(
        ...
        children: <Widget>[
          SimpleDialogOption(
            child: Text(
              "Delete",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold),
  ...
  {
    postsReference.document(ownerId).collection("userPosts").document(postId).get().
    then((doc) {
     if(doc.exists)
     {
 ...
 addLike()
 {
  ...
  if (isNotPostOwner) 
  {
    activityFeedReference.document(ownerId).collection("feedItems").document(postId).setData({
     ...
     controlUserLikePosts ()
     {
       bool _likes = likes[currentOnlineUserId] == true;
       if(_likes)
       {
         postsReference.document(ownerId).collection("userPosts").document(postId).updateData({"likes.$currentOnlineUserId":false});
         removeLike();
         setState(() {
       ...
       {
        postsReference.document(ownerId).collection("userPosts").document(postId).updateData({"likes.$currentOnlineUserId":true});
        addLike();
        setState(() {         
          likeCount = likeCount + 1;
          isLiked = true;
          ...
          setState(() {
            ...
            createPostPicture()
            {
              return GestureDetector(
               onDoubleTap: ()=>controlUserLikePosts(),
               child: Stack(
